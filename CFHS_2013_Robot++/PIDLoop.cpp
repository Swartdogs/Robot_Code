...
PIDLoop::PIDLoop(float Kp, float Ki, float Kd)
{
	m_P = Kp;
	m_I = Ki;
	m_D = Kd;
...
float PIDLoop::Calculate(float input) 							// CALCULATE OUTPUT
{
...
void PIDLoop::Reset()											// RESET CALCULATION DATA
{
	m_prevError = 0;
	m_totalError = 0;
	m_result = 0;
...
void PIDLoop::SetInputRange(float minimumInput, float maximumInput)			// SET INPUT RANGE
{
	m_minimumInput = minimumInput;
	m_maximumInput = maximumInput;
}

void PIDLoop::SetOutputRange(float minimumOutput, float maximumOutput)		// SET OUTPUT RANGE
{
	m_minimumOutput = minimumOutput;
	m_maximumOutput = maximumOutput;
}

void PIDLoop::SetPID(float p, float i, float d)					// SET CALCULATION COEFFICENTS
{
	m_P = p;
	m_I = i;
	m_D = d;
...
void PIDLoop::SetSetpoint(float setpoint)						// SET CALCULATION SETPOINT
{
