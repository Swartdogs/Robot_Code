...
VisionTarget::VisionTarget() : Subsystem("VisionTarget") {
	m_Camera = NULL;
}
    
void VisionTarget::InitDefaultCommand() {
	// Set the default command for a subsystem here.
	//SetDefaultCommand(new MySpecialCommand());
}

void VisionTarget::StartCamera(const char *cameraIP)				// START CAMERA TASK
{
	if (m_Camera == NULL) m_Camera = &AxisCamera::GetInstance(cameraIP);
	
}

void VisionTarget::FindTargets() {
	
	double StartTime = GetClock() * 1000;
	
	printf("Start Find\n");
	
	printf("Fresh Image=%d\n", m_Camera->IsFreshImage());
...
////	Threshold threshold(50, 120, 70, 255, 70, 255);	//HSV threshold criteria, ranges are in that order ie. Hue is 60-100
	ParticleFilterCriteria2 criteria[] = {
			{IMAQ_MT_AREA, AREA_MINIMUM, 65535, false, false}
	};												//Particle filter criteria, used to filter out small particles
//
	BinaryImage *thresholdImage = image->ThresholdHSL(50, 120, 70, 255, 70, 255);
...
double VisionTarget::computeDistance (BinaryImage *image, ParticleAnalysisReport *report) {
 ...
 */
double VisionTarget::scoreAspectRatio(BinaryImage *image, ParticleAnalysisReport *report, bool vertical){
 ...
 */
bool VisionTarget::scoreCompare(Scores scores, bool vertical){
	bool isTarget = true;
 ...
 */
double VisionTarget::scoreRectangularity(ParticleAnalysisReport *report){
 ...
 */
double VisionTarget::ratioToScore(double ratio)
{
 ...
 */
bool VisionTarget::hotOrNot(TargetReport target)
{
	bool isHot = true;
